...
collect_results()
{
	...
	do
        ...
        then
		    brief_result_line=$( python2.6 "$main_dir/helper_scripts/score_extractor.py" "$tested_submission/test_log.txt" "$tested_submission" )
...
prepare_maven_idempotent()
{
    ...
    if ! type "mvn" > /dev/null # if maven is not loaded
    then
        module load maven
    fi
}
